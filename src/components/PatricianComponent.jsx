const PatricianComponent = () => {
  return (
    <section className="pl-3 py-4 h-[30vh] flex flex-col items-center justify-center">
      <dl className="md:max-w-full border-l-4 border-white-800 pl-3">
        <dt className="text-2xl font-serif font-semibold tracking-wide">
          Patrician
        </dt>
        <dd className="mt-3 text-base leading-relaxed font-light font-serif">
          Figura suprema de la élite, forjada en la cima del orden social, cuya sola presencia encarna poder, prestigio y autoridad.
        </dd>
      </dl>
    </section>
  )
}

export default PatricianComponent